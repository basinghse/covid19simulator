...
risk<-data.frame(read.csv("risk.csv"))
...
single_timeseries_plot = function(variable) {
  for (i in 1:ncol(risk)) {
    if(colnames(risk[i])==variable)
    {
      value=as.numeric(risk[,i])
    }
  }
  
  risk$Date=as.Date(risk$Date)
  # Base plot with date axis
  ggplot(risk, aes(x=Date)) + 
...
comparison_plot = function(epi_comp, comparison) {
  ...
  p1 <- ggplot(epi_comp, aes(x = outbreak, y = outcome, fill=outbreak, text = paste0(outbreak, ": ",outcome))) + geom_bar(alpha = 0.8, stat="identity") +
    ...
    scale_fill_manual(values=c("2019-COVID"=covid_col, "2003-SARS"=sars_col)) +
...
basemap = leaflet(plot_map) %>% 
  ...
  addLayersControl(
    ...
    overlayGroups = c("2019-COVID (new)", "2019-COVID (cumulative)", "2003-SARS"),
  ...
  addLegend("bottomright", pal = cv_pal, values = ~cv_large_countries$deathsper100k,
            title = "<small>Units per 100,000</small>") 
...
#sars_cases$country =sars_cases$ï..country
colnames(sars_cases)[1] <- "country"
...
#sars_final =subset(sars_final, select = -c(2) )
...
sars_basemap <- leaflet(sars_plot_map) %>% 
  ...
  addLayersControl(
    position = "bottomright",
    overlayGroups = c("2003-SARS (cumulative)", "2019-COVID"),
    options = layersControlOptions(collapsed = FALSE)) %>% 
  hideGroup(c("2019-COVID"))  %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  fitBounds(~-100,-50,~80,80) %>%
...
risk_factor<- array()
countries<-cv_cases$country
risk_countries<-colnames(risk)
for (i in 1:length(countries)){
    for (j in 1:length(risk_countries)){
       if(countries[i]==risk_countries[j])
           {    
              risk_factor[i]<-risk[1,j]
            
              }
       }
 }
dbcol=length(risk_factor)
for(j in 1:dbcol){
      meandata <- mean(na.omit(risk_factor[j]) )
      #risk_factor[j] <- risk_factor[j] %>% replace_na(meandata)
      risk_factor[is.nan(risk_factor)]<-meandata
      risk_factor[is.na(risk_factor)]<-meandata
      }
cv_cases$risk<-risk_factor
### SHINY SERVER ###
tab_01$server  = function(input, output, session) {
  # covid tab 
  ...
  output$my_risk_map <- renderLeaflet({ 
    basemap
  })
  ...
  observeEvent(input$plot_risk_date, {
    leafletProxy("my_risk_map") %>% 
      clearMarkers() %>%
      clearShapes() %>%
     
      addCircleMarkers(data = reactive_db(), lat = ~ latitude, lng = ~ longitude, weight = 1, radius = ~(cases)^(1/5), 
                       fillOpacity = 0.1, color = covid_col, group = "2019-COVID (cumulative)",
                       label = sprintf("<strong>%s (Current Risk Factor)</strong><br/>Risk Rate: %g", reactive_db()$country,reactive_db()$risk) %>% lapply(htmltools::HTML),
                       labelOptions = labelOptions( 
                         style = list("font-weight" = "normal", padding = "3px 8px", "color" = covid_col),
                         textsize = "15px", direction = "auto"))  %>%  
      addPolygons(data = reactive_polygons(), stroke = FALSE, smoothFactor = 0.1, fillOpacity = 0.15, fillColor = ~cv_pal(reactive_db_large()$deathsper100k)) %>%
  ...
  # country-specific plots
  output$country_Risk_plot_cumulative <- renderPlotly({
